...
for(let i = 1; i < passwordArray.length - 1; i++) {
   const [minMax, letter, pass] = passwordArray[i].split(" ")
   const [min, max] = minMax.split("")
   const filter = pass.split("").filter(element => element === letter.split("").splice(1, 1, ":"))
   
   if (filter.length > max || filter.length < min) {
     invalidPasswords.push([pass])
   }
}

console.log(invalidPasswords[41].join(""))
console.log(invalidPasswords[12].join(""))
